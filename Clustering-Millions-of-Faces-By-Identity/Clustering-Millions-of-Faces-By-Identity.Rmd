...
Having benchmarked the algorithm in contrast to the other methods, the authors estimate internal performance under increasing levels of distractor images and search spaces. This scalability test was then repeated while increasing both the number of distractor images and the number of cores. Each core enabled the code to run in distributed chunks over separate machines.
