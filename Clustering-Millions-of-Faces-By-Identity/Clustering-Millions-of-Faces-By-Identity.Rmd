...
Firstly, the authors improved the Rank-Order clustering algorithm proposed by [@zhu2011]. The original Rank-Order has the disadvantage that it requires $O(n^2)$. The authors propose to use the FLANN library implementation of the randomized k-d tree algorithm to compute the list of top-k nearest neighbors [@hartley2008]. Just one iteration is used. This approximate version had better performance compared to the exact rank-order.
